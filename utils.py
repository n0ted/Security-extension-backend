...
def eval_content_type(contents: str) -> Tuple[int, list]:
    if contents:
        return EVAL_OK, []
    return EVAL_WARN, ["Content-Type header not present"]


def eval_set_cookie(contents: str) -> Tuple[int, list]:
    if contents:
        return EVAL_OK, []
    return EVAL_WARN, ["Set-Cookie header not present"]


def eval_access_control_allow_origin(contents: str) -> Tuple[int, list]:
    if contents:
        return EVAL_OK, []
    return EVAL_WARN, ["Access-Control-Allow-Origin header not present"]


def eval_cross_origin_opener_policy(contents: str) -> Tuple[int, list]:
    if contents:
        return EVAL_OK, []
    return EVAL_WARN, ["Cross-Origin-Opener-Policy header not present"]


def eval_cross_origin_embedder_policy(contents: str) -> Tuple[int, list]:
    if contents:
        return EVAL_OK, []
    return EVAL_WARN, ["Cross-Origin-Embedder-Policy header not present"]


def eval_cross_origin_resource_policy(contents: str) -> Tuple[int, list]:
    if contents:
        return EVAL_OK, []
    return EVAL_WARN, ["Cross-Origin-Resource-Policy header not present"]


def eval_server(contents: str) -> Tuple[int, list]:
    if contents:
        return EVAL_OK, []
    return EVAL_WARN, ["Server header not present"]


def eval_x_powered_by(contents: str) -> Tuple[int, list]:
    if contents:
        return EVAL_OK, []
    return EVAL_WARN, ["X-Powered-By header not present"]


def eval_x_dns_prefetch_control(contents: str) -> Tuple[int, list]:
    if contents:
        return EVAL_OK, []
    return EVAL_WARN, ["X-DNS-Prefetch-Control header not present"]


def eval_public_key_pins(contents: str) -> Tuple[int, list]:
    if contents:
        return EVAL_OK, []
    return EVAL_WARN, ["Public-Key-Pins header not present"]


def eval_x_aspnetmvc_version(contents: str) -> Tuple[int, list]:
    if contents:
        return EVAL_OK, []
    return EVAL_WARN, ["X-AspNetMvc-Version header not present"]


def eval_x_aspnet_version(contents: str) -> Tuple[int, list]:
    if contents:
        return EVAL_OK, []
    return EVAL_WARN, ["X-AspNet-Version header not present"]


def eval_version_info(contents: str) -> Tuple[int, list]:
    if contents:
        return EVAL_OK, []
    return EVAL_WARN, ["Server version header not present"]
